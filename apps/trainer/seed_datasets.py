...
def _augment(df: pd.DataFrame, rows: int, shift: float, rng) -> pd.DataFrame:
    ...
    Шум додаємо ПРОПОРЦІЙНИЙ розкиду кожної ознаки (5% від std), а не однакову
    константу: petal_length змінюється в межах ~6 см, а sepal_width — ~2, і
    однаковий шум спотворив би їх по-різному.
    ...
    for cls, grp in df.groupby("target", sort=True):
        ...
        for f in FEATURES:
            part[f] = part[f] + rng.normal(0, grp[f].std() * 0.05, size=per_class)
        parts.append(part)
