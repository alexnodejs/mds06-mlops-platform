...
порозі MIN_DELTA=0.001 означав, що quality gate Теми 10 порівнював ШУМ.
    ...
    v3 1500 рядків   те саме, але 20% міток versicolor/virginica переплутано

v3 існує заради єдиної демонстрації, якої в курсі досі не було: перетренували на
зіпсованих даних → метрики впали → gate відхилив. Уперше він судить РЕАЛЬНУ
якість даних, а не випадковість на 30 рядках.
...
# 🔴 ЯК САМЕ ЗІПСОВАНИЙ v3 — і чому НЕ зсувом.
#
# Перша версія зсувала petal_length на +0.8 для всіх рядків. Зміряно: модель,
# перетренована на таких даних, дає ТУ САМУ f1 = 0.9800. Це очевидно заднім
# числом: рівномірний зсув однієї ознаки — просто перенос системи координат,
# межі між класами лишаються там само. Зсув ламає модель, яку НАВЧИЛИ на v2 і
# ГОДУЮТЬ v3 (це дріфт на інференсі, і його показує ручка DRIFT_SHIFT у
# генераторі), але перетренування він не псує.
#
# Щоб quality gate мав що відхиляти, псувати треба СИГНАЛ, а не масштаб.
# Найчесніша імітація реальної біди — переплутані мітки: розмітка зламалась на
# джерелі, і 20% versicolor/virginica помінялись місцями. Такі дані знижують
# досяжну якість для БУДЬ-ЯКОЇ моделі, скільки її не тренуй.
LABEL_NOISE = float(os.getenv("V3_LABEL_NOISE", "0.20"))
...
def _corrupt_labels(df: pd.DataFrame, frac: float, rng) -> pd.DataFrame:
    """Міняє місцями мітки versicolor/virginica у частині рядків.

    Саме ці два класи перекриваються в Iris — переплутати setosa з рештою
    неможливо навіть випадково, тож псування там виглядало б штучно.
    """
    out = df.copy()
    pair = ["versicolor", "virginica"]
    idx = out.index[out["target"].isin(pair)]
    n = int(len(idx) * frac)
    victims = rng.choice(idx, size=n, replace=False)
    out.loc[victims, "target"] = out.loc[victims, "target"].map(
...
def build() -> dict[str, pd.DataFrame]:
    ...
    return {
        ...
        # v3 — та сама генерація, що v2, але з переплутаними мітками.
        "v3.csv": _corrupt_labels(_augment(base, BIG, 0.0, rng), LABEL_NOISE, rng),
    }
