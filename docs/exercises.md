...
IMG=$(aws sts get-caller-identity --query Account --output text).dkr.ecr.eu-central-1.amazonaws.com/mds06-mlflow-tools:v6
...
- Якщо Job у `ImagePullBackOff`: приватний ECR працює без `imagePullSecret`
  ...
  (`mds06-mlflow-tools:v6`) — зібрати: `make images`.
...
**Evidently НЕМА в образі `mds06-mlflow-tools:v6`** — і це свідомо: +500-700 MiB
...
docker buildx build --platform linux/amd64 -f apps/trainer/Dockerfile \
  -t $REG/mds06-mlflow-tools:v6 --push .        # 8-12 хв через QEMU

# ручний прогін на новому образі
TRAINER_IMAGE=$REG/mds06-mlflow-tools:v6 make train

# пайплайн: `make pipeline-up` змінних не передає, тому один раз явно
cd terraform/training-pipeline && terraform apply -var "trainer_image=$REG/mds06-mlflow-tools:v6"
