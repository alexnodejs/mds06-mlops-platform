...
> **Перевірити визначення state machine, не застосовуючи його:**
> ```bash
> aws stepfunctions validate-state-machine-definition \
>   --definition file://<відрендерений asl>.json --type STANDARD
> ```
> Повертає `"result": "OK"` і список діагностик. Дешевше, ніж ловити помилку
> ASL під час `terraform apply`.
...
── запускаю cli-20260819-114548 ──
   ...
   ✅ ПРОМОУТ  —  f1 0.9333 проти 0.8997 у чинної: приріст +0.0336 ≥ поріг 0.001
      версія моделі: 4   f1: 0.9333
```

Увесь прогін — близько 90 секунд, з яких саме тренування ~25 c. Решта — опитування
статусу Job: Step Functions перевіряє його приблизно раз на хвилину, і швидше
не буде навіть для миттєвого тренування.
...
**Числа нижче зміряні на реальних прогонах**, а не придумані.

### Прогін 1 — свідомо гірша модель

```bash
make pipeline-run N=10 D=1
```

Десять дерев глибиною 1. Результат:

```
⛔ ВІДХИЛЕНО  —  f1 0.5556 проти 0.9333 у чинної: приріст -0.3778 < поріг 0.001
   версія моделі: 5   f1: 0.5556
   у проді лишилась чинна модель, реєстр поповнився версією 5
```

Показати чотири речі:

1. Виконання **SUCCEEDED**, не FAILED. Пайплайн спрацював рівно як мав.
...
4. `kubectl -n mlflow get jobs` — Job промоції **не створювався взагалі**.
   Гілка `Choice` до нього просто не дійшла.

### Прогін 2 — нормальна модель

```bash
make pipeline-run
```

Сітка за замовчуванням знаходить f1 = 0.9333 → `ПРОМОУТ`. Оновити
http://localhost:8000: номер версії змінився без жодного `kubectl`.

У логах Job промоції видно обидва кроки:

```json
{"event": "alias_moved", "model": "iris-rf", "alias": "champion", "version": "4", "previous": "3"}
{"event": "reloaded", "status": 200, "body": "{\"reloaded\":true,\"version\":\"4\",\"source\":\"registry\"}"}
```

Це і є замикання кола: Тема 8 показувала модель, Тема 9 — реєстр, Тема 10
звʼязала їх пайплайном.

### Прогін 3 — та сама модель ще раз

```bash
make pipeline-run
```

Найцікавіший випадок, і його легко проґавити:

```
⛔ ВІДХИЛЕНО  —  f1 0.9333 проти 0.9333 у чинної: приріст +0.0000 < поріг 0.001
```

Модель **не гірша**. Вона точно така сама — ті самі дані, ті самі гіперпараметри,
той самий `random_state`. І gate її не пускає, бо приріст нульовий.

Питання до групи: чому це правильно? Кожна промоція — це перекат прода: поди
перечитують модель, є вікно, коли частина реплік на старій версії, а частина на
новій. Платити цим ризиком за нуль покращення немає сенсу. Саме тому `MIN_DELTA`
не дорівнює нулю.
...
```
❌ FAILED
ParamsRejected   Параметри з CI не пройшли перевірку.
```

Падає за 200 мс на першому кроці, під не піднімався. Порівняти з тим, скільки це
коштувало б, якби перевірка стояла всередині `train.py`.

### Прогін 5 — через GitHub Actions

**Actions → Тренування моделі → Run workflow**, підставити гіперпараметри.

У логах кроку «Хто ми тепер в AWS» видно доказ до слайда 32:

```json
"Arn": "arn:aws:sts::832828869208:assumed-role/mds06-github-actions/gh-32239775437"
```

Ключів у репозиторії немає жодного, а доступ є — і рівно на цей один job.

⚠️ Зверніть увагу: якщо модель **відхилено**, білд усе одно **зелений**. Червоним
має бути зламаний пайплайн, а не робоче рішення «не беремо цю модель».
...
Більше вправ — у [exercises.md](exercises.md).
