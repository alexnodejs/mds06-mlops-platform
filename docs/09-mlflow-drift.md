...
Збирає й пушить три образи з `apps/`: **`mds06-mlflow-tools:v4`**,
...
Один образ `mds06-mlflow-tools:v4` на три задачі:
...
> 🔴 **Це число змінилось у Темі 11: тепер `JITTER=0.05`.** Джерело правди —
> код (`k8s/model-api/loadgen.yaml`), а не цей абзац.
>
> 0.25 було правильним, доки еталоном служили **120 рядків чистого
> `load_iris()`**. Тема 11 перенесла і тренування, і еталон на
> `s3://datasets/iris/v2.csv` — 1200 рядків, які **самі несуть шум** 0.5 від
> внутрішньокласового std. Джитер згори став надлишковим: для `petal_width`
> власний розкид ≈0.1 см, тобто 0.25 удвічі ширший за сигнал. KS на 1200 проти
> 2400 точок ловить це впевнено — `p = 6e-05`, і дріфт горів **на спокійному
> трафіку**.
>
> Зміряно після зміни: усі чотири ознаки чисті, p-value 0.42…0.87.
>
> **Урок ширший за це число:** джитер — не константа «на смак», а калібрування
> ПІД КОНКРЕТНИЙ еталон. Змінили датасет — переміряйте джитер. І ще ширший:
> еталон і поточні дані мусять походити з ОДНОГО джерела, інакше ви міряєте не
> дріфт, а власну неузгодженість — причому виглядає це правдоподібно.

Історична довідка, чому 0.25 було обрано тоді (еталон = 120 рядків sklearn):
...
apps/                    код і Dockerfile
  trainer/
    ...
    Dockerfile               КОНТРАКТНИЙ образ mds06-mlflow-tools:v4 (обидва скрипти),
